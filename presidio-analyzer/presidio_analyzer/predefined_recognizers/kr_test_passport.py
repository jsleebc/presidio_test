...
def evaluate_passport_recognizer(test_file: str = 'passport_test_cases_1000.json'):
    """
    개선된 여권번호 인식기 평가
    """
    ...
    recognizer = KRPassportRecognizer()
    ...
    for i, case in enumerate(test_cases, 1):
        ...
        if results:
            ...
            passport_valid = (
                recognizer._validate_format(passport_number) and
                recognizer._validate_checksum(passport_number)
            )
            
            if passport_valid:
                score = result.score
                detected_passport = detected_text
                # 체크섬이 유효한 경우에만 True로 설정
                found_valid_passport = result.score >= 0.6 and passport_valid
        
        # 결과 저장
        y_true.append(is_valid)
        y_pred.append(found_valid_passport)
        ...
        result_detail = {
            ...
            "predicted_valid": found_valid_passport,
            "correct_prediction": is_valid == found_valid_passport,
            "detected_passport": detected_passport,
            "score": score
        }
        ...
        if not is_valid == found_valid_passport:
            error_logs.append({
                "case": result_detail,
                "validation_details": {
                    "format_valid": recognizer._validate_format(passport_number),
                    "checksum_valid": recognizer._validate_checksum(passport_number)
                }
            })
    ...
    # 결과 저장
    result_filepath = os.path.join(test_dir, 'evaluation_passport_results.json')
    save_results = {
        "metrics": metrics,
        "error_analysis": {
...
def main():
    try:
        ...
        metrics, error_analysis, detailed_results = evaluate_passport_recognizer()
